...
def load_and_preprocess_lending_club(n_samples=None, random_state=42):
    ...
    if n_samples:
        ...
        skip = sorted(
            np.random.RandomState(random_state).choice(
                range(1, total_rows + 1), total_rows - n_samples, replace=False
            )
        )
    ...
    if "fico_range_low" in df.columns and "fico_range_high" in df.columns:
        ...
        grade_to_fico = {"A": 750, "B": 700, "C": 650, "D": 600, "E": 550, "F": 500, "G": 450}
    ...
    result = df[
        ["loan_id", "income", "total_debt", "fico_score", "loan_amount", "default_flag"]
    ].copy()
