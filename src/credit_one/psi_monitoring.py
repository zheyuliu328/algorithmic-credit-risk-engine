...
class PSIMonitor:
    ...
    THRESHOLDS = {"stable": 0.10, "warning": 0.25, "critical": float("inf")}
    ...
    def calculate_psi(
        self, current_scores: np.ndarray, buckets: int = 10, bucket_type: str = "quantile"
    ) -> Dict:
        """
        ...
        if bucket_type == "quantile":
            ...
            breakpoints = np.percentile(baseline_scores, np.linspace(0, 100, buckets + 1))
        else:
            # Equal width buckets
            breakpoints = np.linspace(baseline_scores.min(), baseline_scores.max(), buckets + 1)
    ...
    def log_psi(
        self, psi_results: Dict, period_start: str = None, period_end: str = None, notes: str = ""
    ):
        """
        ...
        print(
            f"✓ PSI logged: {psi_results['alert_color']} {psi_results['alert_level']} "
...
"""
        ...
        sorted_buckets = sorted(
            psi_results["bucket_details"], key=lambda x: abs(x["psi_contribution"]), reverse=True
        )[:5]
