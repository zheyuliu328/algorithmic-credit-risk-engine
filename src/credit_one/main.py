...
def step1_raw_data_injection(conn, use_real_data=False, n_samples=10000):
    ...
    if use_real_data and LENDING_CLUB_AVAILABLE:
        ...
        default_prob = 1 / (
            1 + np.exp(-(-3 + 2 * dti_ratio - 0.01 * fico_score + 0.0001 * loan_amount))
        )
...
def step3_python_modeling(conn):
    ...
    all_predictions = model.predict_proba(
        df[["dti_ratio", "fico_encoded", "loan_to_income_ratio", "loan_size_encoded"]]
    )[:, 1]
    ...
    predictions_df = pd.DataFrame({"loan_id": loan_ids["loan_id"], "pd": all_predictions})
