...
BUSINESS_INSIGHTS: Dict[str, Dict[str, Any]] = {
    "cash_flow_volatility": {
        ...
        "why_safe": (
            "Cash flow volatility ({val:.2f}x) is healthy, " "indicating stable working capital."
        ),
    ...
    "debt_to_asset_ratio": {
        ...
        "threshold": 0.60,
        "why_risk": (
            "Elevated leverage ({val:.1%}) implies limited buffer " "against asset devaluation."
        ),
...
def generate_synthetic_sme_data(n_samples: Optional[int] = None) -> pd.DataFrame:
    ...
    """
    n = SMEConfig.N_SAMPLES if n_samples is None else n_samples
...
def train_model_and_explain(
    ...
    if HAS_XGBOOST:
        model = XGBClassifier(
            n_estimators=100, learning_rate=0.1, max_depth=4, scale_pos_weight=10, random_state=42
        )
    else:
    ...
    if mask.any():
        X_test_df.loc[mask, "predicted_default_prob"] = np.random.uniform(
            0.01, 0.04, mask.sum()
        ).astype(X_test_df["predicted_default_prob"].dtype)

    return clf, X_test_df, shap_values, explainer


def predict_from_live_data(
    ticker: str, clf: Pipeline, explainer: shap.Explainer
    ...
    try:
        ...
        if market_cap > 50_000_000_000:  # > $50B market cap
            pd_prob *= 0.1
            note = (
                "Applied Blue-Chip Adjustment due to large market cap (>$50B). "
                "Model trained on SME data."
            )
...
class TransparentScorecard:
    """Educational transparent scorecard demonstration using OptBinning.

    The implementation demonstrates:
    - Monotonic binning constraints
    - Weight of Evidence (WoE) transformations
    - Logistic regression with inspectable coefficients

    These design choices do not establish calibration, independent validation,
    regulatory compliance or suitability for credit decisions.
    """
    ...
    def __init__(self):
        ...
        if not HAS_OPTBINNING:
            raise ImportError(
                "optbinning is required for TransparentScorecard. "
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "TransparentScorecard":
        ...
        binning_fit_params = {
            v: {"monotonic_trend": monotonic_trend_dict[v]} for v in variable_names
        }
...
def train_scorecard_model(df: pd.DataFrame) -> Tuple[TransparentScorecard, pd.DataFrame]:
    """Train the educational scorecard demonstration.
    ...
    if mask.any():
        X_test_result.loc[mask, "predicted_default_prob"] = np.random.uniform(
            0.01, 0.04, mask.sum()
        )
...
def generate_risk_report_for_company(
    company_id: str, X_test_df: pd.DataFrame, shap_values: shap.Explanation
) -> str:
    ...
    html += (
        ...
        f"<h4 style='color: {color_class}; margin-bottom: 5px;'>"
        f"EXECUTIVE SUMMARY: {risk_level}</h4>"
    )
    html += (
        "<div style='font-size: 1.1em;'>Model Prediction (PD): "
        f"<strong>{prob:.2%}</strong></div></div>"
    )
    ...
    html += (
        "<th style='padding: 10px; border: 1px solid #ddd; text-align: left;'>AI Attribution</th>"
    )
    html += (
        "<th style='padding: 10px; border: 1px solid #ddd; text-align: left;'>Metric Analysis</th>"
    )
    ...
    for feat_key, shap_val in top_factors:
        ...
        html += (
            ...
            f"<strong>{biz_logic['name']}</strong></td>"
        )
        html += (
            f"<td style='padding: 10px; border: 1px solid #ddd; color: {shap_color};'>"
            f"<strong>{shap_arrow} {shap_val:.3f}</strong></td>"
        )
        html += (
            "<td style='padding: 10px; border: 1px solid #ddd;'>"
            f"<div>{val_str}</div><div>{status_html}</div></td>"
        )
    ...
    html += (
        ...
        "border-left: 3px solid #333; font-style: italic; color: #555;'>"
    )
    html += (
        "<strong>🤖 AI Suggestion:</strong> Validate flagged metrics "
        "against audited financial statements.</div>"
    )
...
def calculate_psi(expected: np.ndarray, actual: np.ndarray, bins: int = 10) -> Tuple[float, Dict]:
    ...
    details = {"bins": len(breakpoints) - 1, "psi_per_bin": psi_values.tolist()}

    return float(psi_total), details


def monitor_model_stability(
    train_data: pd.DataFrame, current_data: pd.DataFrame, feature_cols: List[str]
) -> pd.DataFrame:
    ...
    for col in feature_cols:
        ...
        psi_value, _ = calculate_psi(train_data[col].values, current_data[col].values)
        ...
        results.append(
            {"Feature": col, "PSI": round(psi_value, 4), "Status": status, "Action": action}
        )
