...
def main() -> None:
    ...
    with col2:
        ...
        k1.metric(
            "Revenue Growth",
            f"{str_rev:.1%}",
            delta=f"{shock_rev:.0%} Shock",
            delta_color="inverse",
        )
        k2.metric("Debt-to-Asset", f"{row['debt_to_asset_ratio']:.1%}", help="Threshold: 60%")
        k3.metric(
            "CF Volatility", f"{str_vol:.2f}", delta=f"{shock_vol:.1f}x Amp", delta_color="inverse"
        )
        ...
        if risks:
            ...
            insight_text = (
                " ".join(risks)
                + " These factors significantly elevate the Probability of Default under stress."
            )
    ...
    with tab2:
        ...
        shap_fig = go.Figure(
            go.Bar(x=df_shap["SHAP"], y=df_shap["Feature"], orientation="h", marker_color=colors)
        )
        shap_fig.update_layout(
            height=300, margin=dict(l=0, r=0, t=0, b=0), xaxis_title="Risk Contribution (Log-odds)"
        )
