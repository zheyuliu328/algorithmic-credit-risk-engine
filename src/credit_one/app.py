...
def init_ui_style():
    ...
    st.markdown(
        ...
        .insight-box-risk { background-color: #ffebee; border-left: 5px solid #d32f2f;
            padding: 15px; border-radius: 4px; color: #b71c1c; }
        .insight-box-safe { background-color: #e8f5e9; border-left: 5px solid #2e7d32;
            padding: 15px; border-radius: 4px; color: #1b5e20; }
        .report-container { background-color: white; padding: 30px; border: 1px solid #ccc;
            font-family: 'Times New Roman', serif; margin-top: 20px; }
        .section-header {
            color: #88c0d0;
            font-size: 14px;
            font-weight: 700;
...
def _generate_live_report(metrics: Dict[str, Any], shap_vals) -> str:
    ...
    html += "<tr><td style='padding:10px;border:1px solid #ddd;'>Revenue Growth</td>"
    ...
    html += "<tr><td style='padding:10px;border:1px solid #ddd;'>Debt-to-Asset Ratio</td>"
    ...
    html += "<tr><td style='padding:10px;border:1px solid #ddd;'>Cash Flow Volatility</td>"
...
def render_analysis_tab(clf, explainer, scorecard):
    ...
    if result is None:
        center_col.info(
            "👆 Enter a ticker and click **Analyze** to start real-time risk assessment."
        )
        return
    ...
    with left_col:
        ...
        st.metric(
            "Revenue Growth", f"{str_rev:.1%}", delta=f"{shock_rev:.0%}" if shock_rev != 0 else None
        )
        st.metric("Debt-to-Asset", f"{row['debt_to_asset_ratio']:.1%}")
        st.metric(
            "CF Volatility",
            f"{str_vol:.2f}",
            delta=f"{shock_vol:.1f}x" if shock_vol != 1.0 else None,
            delta_color="inverse",
        )
    ...
    with center_col:
        ...
        if use_scorecard and scorecard is not None:
            ...
            st.markdown(
                '<div class="section-header">🎯 PROBABILITY OF DEFAULT</div>',
                unsafe_allow_html=True,
            )
            fig = go.Figure(
                go.Indicator(
        ...
        st.markdown(
            '<div class="section-header">📈 MARKET PRICE TREND</div>', unsafe_allow_html=True
        )
        price_fig = go.Figure(
            data=[go.Scatter(x=hist.index, y=hist["Close"], line=dict(color="#1976d2", width=2))]
        )
        price_fig.update_layout(
            height=250, margin=dict(l=0, r=0, t=0, b=0), template="plotly_white", showlegend=False
        )
    ...
    with right_col:
        ...
        if use_scorecard and scorecard is not None:
            ...
            try:
                ...
                st.dataframe(
                    binning_df[["Bin", "Count", "Event rate", "WoE"]].head(5),
            ...
            feature_names: List[str] = [
                f.replace("industry_", "Sector: ") for f in sv.feature_names
            ]
            ...
            shap_fig = go.Figure(
                go.Bar(
                    x=df_shap["SHAP"], y=df_shap["Feature"], orientation="h", marker_color=colors
                )
            )
        ...
        credit_rating = (
            "Investment Grade"
            if new_pd < 0.15
            else "Speculative Grade" if new_pd < 0.30 else "High Risk"
        )
...
def render_monitoring_tab():
    ...
    with col1:
        uploaded_file = st.file_uploader(
            ...
            type=["csv"],
            help="Upload real production data to detect actual drift",
        )
        if uploaded_file:
            try:
    ...
    with col2:
        if st.button("🎲 Simulate Drift (Demo)"):
            ...
            drifted_data["revenue_growth"] = train_data["revenue_growth"] + np.random.normal(
                0.02, 0.01, len(train_data)
            )
            drifted_data["debt_to_asset_ratio"] = train_data[
                "debt_to_asset_ratio"
            ] + np.random.normal(0.15, 0.05, len(train_data))
            drifted_data["cash_flow_volatility"] = train_data[
                "cash_flow_volatility"
            ] * 1.5 + np.random.normal(0, 0.2, len(train_data))
    ...
    if data_source == "baseline":
        st.info(
            "ℹ️ **Baseline Mode**: Comparing training data with itself (PSI = 0). "
            "Upload production data or simulate drift to see monitoring in action."
        )
    elif data_source == "uploaded":
        st.success(
            f"✅ **Production Mode**: Monitoring {len(current_data)} production records "
            f"against {len(train_data)} baseline records."
        )
    elif data_source == "simulated":
        st.warning(
            "⚠️ **Demo Mode**: Using simulated drift for demonstration. "
            "Upload real data for actual monitoring."
        )
    ...
    if max_psi >= 0.25:
        st.error(
            "🚨 **CRITICAL ALERT**: Significant data drift detected! Model retraining is required."
        )
    elif max_psi >= 0.1:
        st.warning(
            "⚠️ **WARNING**: Moderate data drift detected. Monitor model performance closely."
        )
    else:
    ...
    colors = [
        "red" if psi >= 0.25 else "orange" if psi >= 0.1 else "green" for psi in psi_df["PSI"]
    ]
    ...
    with st.expander("ℹ️ How to interpret PSI scores"):
        st.markdown(
            """
        **Population Stability Index (PSI)** measures the shift in data distribution
